...
python () {
    flavor = d.getVar('RDK_FLAVOR')
    if flavor == 'rdkv':
        d.setVar('SRC_URI_remove', '${RDK_GENERIC_ROOT_GIT}/gstreamer-netflix-platform/generic;protocol=${RDK_GIT_PROTOCOL};branch=${RDK_GIT_BRANCH}')
    else:
        d.setVar('SRC_URI_remove', '${CMF_GITHUB_ROOT}/gstreamer-netflix-platform;${CMF_GITHUB_SRC_URI_SUFFIX}')
}
